...
function BlockQuickLook ({ blockData }) {
  ...
  return (
    <div className='fixed top-0 left-0 h-screen w-screen z-50 p-8 cursor-pointer' onClick={close}>
      <div className='relative h-full w-full overflow-hidden flex justify-center items-center z-50 bg-black bg-opacity-80 backdrop-blur-md border-2 border-secondary rounded-sm'>
        {!imageLoaded && <Spinner />}
        ...
        <div className="absolute top-0 right-0 text-primary border-secondary border-2 m-2 p-4 rounded-sm bg-black bg-opacity-80 backdrop-blur-md text-right">
