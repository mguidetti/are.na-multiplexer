...
function Header () {
  ...
  return (
    <div className='flex gap-x-4 items-start bg-background text-primary/70'>
      <div className='w-64 flex align-middle gap-x-2 p-2'>
        <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 150.38 88.986' fill='currentColor' className='w-8'>
      ...
      <div className='w-64 text-right flex gap-x-4 justify-end items-center p-2'>
        {data && (
          <>
            {data?.user?.name}
            <button onClick={() => signOut()} className="py px-3 border border-primary rounded-lg hover:bg-secondary/50">Sign out</button>
          </>
        )}
      </div>
    </div>
