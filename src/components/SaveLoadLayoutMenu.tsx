...
function SaveLoadLayout () {
  ...
  const { saveLayout, savedLayouts, setDialog, removeSavedLayout, restoreLayout, channels } = useDesktopContext()

  const handleSave = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault()

    saveLayout(newSaveName)
    setOpen(false)
    setNewSaveName('')
  }

  const handleRestore = (layoutId: string) => {
    setDialog({
      isOpen: true,
      title: `Are you sure you want to load ${savedLayouts[layoutId].name}?`,
  ...
  const handleDelete = (layoutId: string) => {
    setDialog({
      isOpen: true,
      title: `Are you sure you want to delete ${savedLayouts[layoutId].name}?`,
      message: 'This is not undoable',
      onConfirm: () => {
        removeSavedLayout(layoutId)
      }
    })
  }

  const renderSavedLayouts = () => {
    if (savedLayouts && Object.keys(savedLayouts).length) {
      return (
        ...
        <div>
          ...
          <form
            ...
            className={classNames('flex mt-2 gap-x-3 items-center', {
              'pointer-events-none opacity-25': !Object.keys(channels).length
            })}
          >
            <input
              ...
              disabled={!Object.keys(channels).length}
              required={true}
            />
